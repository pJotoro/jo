...
shader_binary :: proc "contextless" (shaders: []Shader, binary_format: Binary_Format, binary: []byte) {
	gl.impl_ShaderBinary(i32(len(shaders)), ([^]u32)(raw_data(shaders)), u32(binary_format), raw_data(binary), i32(len(binary)))
}
...
program_binary :: proc "contextless" (program: Program, binary_format: Binary_Format, binary: []byte) {
	gl.impl_ProgramBinary(u32(program), u32(binary_format), raw_data(binary), i32(len(binary)))
}
